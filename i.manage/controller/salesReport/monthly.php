...
SUM(orders.wprice * orders.qty) AS wholeSale,
COUNT(orders.qty) AS totalitems
FROM order_records  INNER JOIN orders
ON order_records.id	=	orders.order_recID
WHERE
MONTH(order_records.date) = MONTH(CURRENT_DATE())
AND YEAR(order_records.date) = YEAR(CURRENT_DATE())
...
if($_POST['query'] != '')
{
  ...
  OR MONTH(order_records.date) = MONTH(CURRENT_DATE())
  AND YEAR(order_records.date) = YEAR(CURRENT_DATE())
  AND	order_records.loan_status !='Loan'  AND order_records.name LIKE '%$text%'
  OR MONTH(order_records.date) = MONTH(CURRENT_DATE())
  AND YEAR(order_records.date) = YEAR(CURRENT_DATE())
  AND order_records.loan_status !='Loan' 	AND order_records.pnum LIKE '%$text%'
  ";
}
