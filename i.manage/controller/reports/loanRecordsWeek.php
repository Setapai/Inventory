...
DATE_FORMAT(order_records.date, '%W')  AS dateTime,
order_records.loan_status
FROM order_records
INNER JOIN loans ON order_records.id = loans.order_recID
WHERE YEARWEEK(loans.dateTime, 1) = YEARWEEK(CURDATE(), 1) AND order_records.loan_status = 'Loan'
";
...
$output = '
<label>This Week Loans Records - '.$total_data.'</label>
